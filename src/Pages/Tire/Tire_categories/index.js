...
// Actions
import * as actions from "../../../redux/actions/tireCategoryActions";
...
const WheelCategories = (props) => {
  ...
  const editMenu = (values) => {
    props.updateTireCategory(values, select[0]);
    handleCancel();
  };

  const deleteMenu = () => {
    props.deleteTireCategory(select[0], selectData);
    setSelect([]);
    setSelectData({});
    handleCancel();
  };
...
const mapStateToProps = (state) => {
  return {
    success: state.tireCategoryReducer.success,
    error: state.tireCategoryReducer.error,
    loading: state.tireCategoryReducer.loading,
    categories: state.tireCategoryReducer.categories,
    category: state.tireCategoryReducer.category,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    createCategory: (data) => dispatch(actions.saveTireCategory(data)),
    loadTireCategories: () => dispatch(actions.loadTireCategories()),
    getCategory: (id) => dispatch(actions.loadTireCategory(id)),
    changePosition: (data) => dispatch(actions.changePosition(data)),
    updateTireCategory: (data, id) =>
      dispatch(actions.updateTireCategory(data, id)),
    deleteTireCategory: (id) => dispatch(actions.deleteTireCategory(id)),

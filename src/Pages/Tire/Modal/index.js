...
const Make = (props) => {
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/tire/modal/edit/${selectedRowKeys[0]}`);
    }
  };

  const handleDelete = () => {
    props.deleteMultTireModal(selectedRowKeys);
  };
  ...
  useEffect(() => {
    if (props.tiremodals) {
      ...
      props.tiremodals.length > 0 &&
        props.tiremodals.map((el) => {
          const key = el._id;
          delete el._id;
    ...
    switch (modal) {
      ...
      case "edit": {
        if (selectedRowKeys && selectedRowKeys.length === 1) {
          props.history.replace("/tire/modal/edit/" + selectedRowKeys[0]);
        } else {
          toastControl("error", "Нэг өгөгдөл сонгоно уу");
        }
        break;
  ...
  const exportExcel = async () => {
    const query = queryBuild();
    const response = await axios.get("tiremodals/excel?" + query);
    let excelData = [];
    if (response) {
                ...
                <div className="datatable-header-tools">
                  <div className="datatable-actions">
                    <button
                      ...
                      onClick={() => history.push(`/tire/modal/add`)}
                    >
...
const mapStateToProps = (state) => {
  return {
    ...
    pagination: state.tireModalReducer.paginationLast,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    loadTireModal: (query) => dispatch(actions.loadTireModal(query)),
    deleteMultTireModal: (ids) => dispatch(actions.deleteMultTireModal(ids)),

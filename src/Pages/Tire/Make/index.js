...
// Actions
import * as actions from "../../../redux/actions/tireMakeActions";
...
const Make = (props) => {
      ...
      render: (text, record) => {
        ...
        return (
          <div className="table-image">
            {record.logo ? (
              <img src={`${base.cdnUrl}${record.logo}`} />
            ) : (
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/tire/make/edit/${selectedRowKeys[0]}`);
    }
  };

  const handleDelete = () => {
    props.deleteMultTireMake(selectedRowKeys);
  };
  ...
  useEffect(() => {
    if (props.tiremakes) {
      ...
      props.tiremakes.length > 0 &&
        props.tiremakes.map((el) => {
          const key = el._id;
          delete el._id;
    ...
    switch (modal) {
      ...
      case "edit": {
        if (selectedRowKeys && selectedRowKeys.length === 1) {
          props.history.replace("/tire/make/edit/" + selectedRowKeys[0]);
        } else {
          toastControl("error", "Нэг өгөгдөл сонгоно уу");
        }
        break;
  ...
  const exportExcel = async () => {
    const query = queryBuild();
    const response = await axios.get("tiremakes/excel?" + query);
    let excelData = [];
    if (response) {
  ...
  return (
    <>
      <div className="content-wrapper">
        <PageTitle name="Дугуй үйлдвэрлэгчид" />
          ...
          <div className="container-fluid">
            <div className="card datatable-card">
              <div className="card-header">
                <h3 className="card-title">Бүх үйлдвэрүүд</h3>
              </div>
                ...
                <div className="datatable-header-tools">
                  <div className="datatable-actions">
                    <button
                      ...
                      onClick={() => history.push(`/tire/make/add`)}
                    >
...
const mapStateToProps = (state) => {
  return {
    loading: state.tireMakeReducer.loading,
    success: state.tireMakeReducer.success,
    error: state.tireMakeReducer.error,
    tiremakes: state.tireMakeReducer.tiremakes,
    pagination: state.tireMakeReducer.paginationLast,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    loadTireMake: (query) => dispatch(actions.loadTireMake(query)),
    deleteMultTireMake: (ids) => dispatch(actions.deleteMultTireMake(ids)),
    getExcelData: (query) => dispatch(actions.getExcelData(query)),
    clear: () => dispatch(actions.clear()),
  };
};

export default connect(mapStateToProps, mapDispatchToProp)(Make);

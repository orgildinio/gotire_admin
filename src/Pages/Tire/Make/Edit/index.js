...
const Edit = (props) => {
  ...
  // FUNCTIONS
  const init = () => {
    props.getTireMake(props.match.params.id);
  };
  ...
  const handleAdd = (values, status = null) => {
    ...
    props.updateTireMake(props.match.params.id, sendData);
  };
  ...
  useEffect(() => {
    if (props.tiremake) {
      form.setFieldsValue({ ...props.tiremake });

      props.tiremake.logo &&
        setLogo({
          name: props.tiremake.logo,
          url: `${base.cdnUrl}${props.tiremake.logo}`,
        });

      setCheckedRadio((bc) => ({
        ...bc,
        status: props.tiremake.status,
      }));
    }
  }, [props.tiremake]);
  ...
  return (
    <>
      <div className="content-wrapper">
        <PageTitle name="Үйлдвэрлэгч засах" />
...
const mapStateToProps = (state) => {
  return {
    ...
    tiremake: state.tireMakeReducer.tiremake,
  };
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    updateTireMake: (id, data) => dispatch(actions.updateTireMake(id, data)),
    getTireMake: (id) => dispatch(actions.getTireMake(id)),

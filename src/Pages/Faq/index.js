...
const Faqs = (props) => {
  ...
  const [columns, setColumns] = useState([
...

    {
      ...
      status: true,
      render: (text, record) => {
        return record.answer.substr(0, 80);
      },
...

    {
      ...
      status: true,
      render: (text, record) => {
        return record.question.substr(0, 80);
      },
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/faqs/edit/${selectedRowKeys[0]}`);
    }
  };

  const handleDelete = () => {
    props.deleteMultFaq(selectedRowKeys);
  };
  ...
  const exportExcel = async () => {
    const query = queryBuild();
    const response = await axios.get("faqs/excel?" + query);
    let excelData = [];
    if (response) {
          ...
          <div className="container-fluid">
            <div className="card datatable-card">
              <div className="card-header">
                <h3 className="card-title">Бүх түгээмэл асуулт</h3>
              </div>
...
const mapStateToProps = (state) => {
  return {
    ...
    pagination: state.faqReducer.paginationLast,
    excelData: state.faqReducer.excelData,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    loadFaq: (query) => dispatch(actions.loadFaq(query)),
    deleteMultFaq: (ids) => dispatch(actions.deleteMultFaq(ids)),
    getExcelData: (query) => dispatch(actions.getExcelData(query)),
    clear: () => dispatch(actions.clear()),
  };
};

export default connect(mapStateToProps, mapDispatchToProp)(Faqs);

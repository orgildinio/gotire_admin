...
const Services = (props) => {
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/services/edit/${selectedRowKeys[0]}`);
    }
  };

  const handleDelete = () => {
    props.deleteMultService(selectedRowKeys);
  };
  ...
  const exportExcel = async () => {
    const query = queryBuild();
    const response = await axios.get("services/excel?" + query);
    let excelData = [];
    if (response) {
      ...
      excelData = data.map((el) => {
        cloneColumns.map((col) => {
          ...
          if (col.key === "direct" && col.status === true) {
            el.direct =
              el.direct && el.direct == true
                ? "Линк холбосон"
                : "Линк холбоогүй";
          }
          ...
          <div className="container-fluid">
            <div className="card datatable-card">
              <div className="card-header">
                <h3 className="card-title">Бүх үйлчилгээ</h3>
              </div>
...
const mapStateToProps = (state) => {
  return {
    ...
    pagination: state.serviceReducer.paginationLast,
    excelData: state.serviceReducer.excelData,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    loadService: (query) => dispatch(actions.loadService(query)),

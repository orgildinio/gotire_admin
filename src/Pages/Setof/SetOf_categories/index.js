...
// Actions
import * as actions from "../../../redux/actions/setofCategoriesActions";
...
const SetOfCategories = (props) => {
  ...
  const editMenu = (values) => {
    props.updateCategory(values, select[0]);
    handleCancel();
  };

  const deleteMenu = () => {
    props.deleteCategory(select[0], selectData);
    setSelect([]);
    setSelectData({});
    handleCancel();
  };
...
const mapStateToProps = (state) => {
  return {
    success: state.setofCategoryReducer.success,
    error: state.setofCategoryReducer.error,
    loading: state.setofCategoryReducer.loading,
    categories: state.setofCategoryReducer.categories,
    category: state.setofCategoryReducer.category,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    createCategory: (data) => dispatch(actions.saveSetProductCategory(data)),
    loadCategories: () => dispatch(actions.loadSetProductCategories()),
    getCategory: (id) => dispatch(actions.loadSetProductCategory(id)),
    changePosition: (data) => dispatch(actions.changePosition(data)),
    updateCategory: (data, id) =>
      dispatch(actions.updateSetProductCategory(data, id)),
    deleteCategory: (id) => dispatch(actions.deleteSetProductCategory(id)),
    clear: () => dispatch(actions.clear()),
  };
};

export default connect(mapStateToProps, mapDispatchToProp)(SetOfCategories);

...
const Paytype = (props) => {
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/paytype/edit/${selectedRowKeys[0]}`);
    }
  };

  const handleDelete = () => {
    props.deleteMultPaytype(selectedRowKeys);
  };
  ...
  useEffect(() => {
    if (props.paytypes) {
      ...
      props.paytypes.length > 0 &&
        props.paytypes.map((el) => {
          const key = el._id;
          delete el._id;
                ...
                <div className="datatable-header-tools">
                  <div className="datatable-actions">
                    <button
                      ...
                      onClick={() => history.push(`/paytype/add`)}
                    >
...
const mapStateToProps = (state) => {
  return {
    ...
    paytypes: state.payTypeReducer.paytypes,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    loadPaytype: (query) => dispatch(actions.loadPaytype(query)),
    deleteMultPaytype: (ids) => dispatch(actions.deleteMultPaytype(ids)),
    clear: () => dispatch(actions.clear()),
  };
};

export default connect(mapStateToProps, mapDispatchToProp)(Paytype);

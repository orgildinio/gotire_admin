...
// Actions
import * as actions from "../../../redux/actions/wheelCategoryActions";
...
const WheelCategories = (props) => {
  ...
  const editMenu = (values) => {
    props.updateWheelCategory(values, select[0]);
    handleCancel();
  };

  const deleteMenu = () => {
    props.deleteWheelCategory(select[0], selectData);
    setSelect([]);
    setSelectData({});
    handleCancel();
  };
...
const mapStateToProps = (state) => {
  return {
    success: state.wheelCategoryReducer.success,
    error: state.wheelCategoryReducer.error,
    loading: state.wheelCategoryReducer.loading,
    categories: state.wheelCategoryReducer.categories,
    category: state.wheelCategoryReducer.category,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    createCategory: (data) => dispatch(actions.saveWheelCategory(data)),
    loadWheelCategories: () => dispatch(actions.loadWheelCategories()),
    getCategory: (id) => dispatch(actions.loadWheelCategory(id)),
    changePosition: (data) => dispatch(actions.changePosition(data)),
    updateWheelCategory: (data, id) =>
      dispatch(actions.updateWheelCategory(data, id)),
    deleteWheelCategory: (id) => dispatch(actions.deleteWheelCategory(id)),
    clear: () => dispatch(actions.clear()),
  };
};

export default connect(mapStateToProps, mapDispatchToProp)(WheelCategories);
